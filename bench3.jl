...
Yex = Matrix(CSV.read("ExactSIR.csv", DataFrame, header = 0)) # Solution with a fine step size from Matlab
## inputs
...
for n in range(2, 7)
    ...
    ery1=norm(y1' .- Yex[:,1:2^(10-n):end],2); push!(E1, ery1)
    ery2=norm(y2' .- Yex[:,1:2^(10-n):end],2); push!(E2, ery2)
    ery3=norm(y3.u .- Yex[:,1:2^(10-n):end],2); push!(E3, ery3)
    ery4=norm(y4.u .- Yex[:,1:2^(10-n):end],2); push!(E4, ery4)
    ery5=norm(y5.u .- Yex[:,1:2^(10-n):end],2); push!(E5, ery5)
    ery6=norm(y6.u .- Yex[:,1:2^(10-n):end],2); push!(E6, ery6)
    ery7=norm(y7.u .- Yex[:,1:2^(10-n):end],2); push!(E7, ery7)
    ery8=norm(y8.u .- Yex[:,1:2^(10-n):end],2); push!(E8, ery8)
    ery9=norm(y9.u .- Yex[:,1:2^(10-n):end],2); push!(E9, ery9)
...
plot!(Mdata[:, 1], Mdata[:, 5], linewidth = 5, markersize = 5,label = "Matlab PI-EX (Garrappa)", shape = :rect, color = :orange)
plot!(Mdata[:, 2], Mdata[:, 6], linewidth = 5, markersize = 5,label = "Matlab PI-PC (Garrappa)", shape = :circle, color = :orange)
plot!(Mdata[:, 3], Mdata[:, 7], linewidth = 5, markersize = 5,label = "Matlab PI-IM1 (Garrappa)", shape = :rect, color = :red)
plotd1=plot!(Mdata[:, 4], Mdata[:, 8], linewidth = 5, markersize = 5,label = "Matlab PI-IM2 (Garrappa)", shape = :circle, color = :red)
savefig(plotd1,"file2.png")
