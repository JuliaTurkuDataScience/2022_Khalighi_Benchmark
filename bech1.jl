...
#########scifracX
FF(t, y) = (40320 ./ gamma(9 - β) .* t .^ (8 - β) .- 3 .* gamma(5 + β / 2)
               ./ gamma(5 - β / 2) .* t .^ (4 - β / 2) .+ 9/4 * gamma(β + 1) .+
               (3 / 2 .* t .^ (β / 2) .- t .^ 4) .^ 3 .- y .^ (3 / 2))
...
Mdata = CSV.read("benchmarks/FDEsolver/Data_Matlab_1D.csv", DataFrame, header = 0) #it should be based on the directory of CSV files on your computer
...
plot(T3, E3, linewidth = 5, yaxis=:log, xscale = :log, label = "Julia GL (FractionalDiffEq.jl)", xlabel="Time (sc, Log)", ylabel="Error: 2-norm (Log)")
plot!(Mdata[!, 3], Mdata[!, 6], linewidth = 5, label = "Matlab PI-EX (Garrappa)")
